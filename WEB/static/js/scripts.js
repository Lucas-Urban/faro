document.addEventListener("DOMContentLoaded", function () {
    ...
    botaoAbrirModalEncontrarPet.addEventListener('click', function () {
        ...
        modal.show();
        getCurrentLocation("latLocalPet", "longLocalPet", "inputLocalPet");
    });
    ...
    botaoAbrirModalbotaoEncontrarTutor.addEventListener('click', function () {
        ...
        modal.show();
        getCurrentLocation("latLocalEncontrarTutor", "longLocalEncontrarTutor", "inputLocalEncontrarTutor");
    });
...
function getCurrentLocation(inputLatitude, inputLongitude, inputLocal) {
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
            function (position) {
                // Define os valores dos campos latLocalEncontrarTutor e longLocalEncontrarTutor
                const latLocalEncontrarTutor = document.getElementById(inputLatitude);
                const longLocalEncontrarTutor = document.getElementById(inputLongitude);
                latLocalEncontrarTutor.value = position.coords.latitude;
                longLocalEncontrarTutor.value = position.coords.longitude;

                // Cria um novo objeto geocoder para obter o endereço correspondente à localização
                const geocoder = new google.maps.Geocoder();
                const latlng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude);
                geocoder.geocode({ 'latLng': latlng }, function (results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        if (results[0]) {
                            // Define o valor do campo inputLocalEncontrarTutor com o endereço correspondente à localização
                            const inputLocalEncontrarTutor = document.getElementById(inputLocal);
                            inputLocalEncontrarTutor.value = results[0].formatted_address;
                        }
                    }
                });
            },
            function (error) {
                // Trata o erro ao solicitar a localização do usuário
                console.error(error);
            }
        );
    } else {
